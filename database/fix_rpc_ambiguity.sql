-- FIX: Drop old functions to resolve ambiguity
-- We need to drop the versions that took 'text' for discipline/source because they conflict with the new 'text[]' versions when passing null.

-- 1. Drop old get_random_questions (Text version)
DROP FUNCTION IF EXISTS public.get_random_questions(text, text, text, text, integer, text);

-- 2. Drop old get_questions_count (Text version)
DROP FUNCTION IF EXISTS public.get_questions_count(text, text, text, text, uuid, boolean, boolean);

-- 3. Ensure the NEW functions (Array version) are definitely there (Re-run creation just to be safe)
